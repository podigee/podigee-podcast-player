...
var $, ChapterMarks, Configuration, Download, Embed, EpisodeInfo, ExternalData, Feed, Player, Playlist, PodigeePodcastPlayer, ProgressBar, Share, Theme, Transcript, _,
...
PodigeePodcastPlayer = (function() {
  PodigeePodcastPlayer.defaultExtensions = [ProgressBar, ChapterMarks, Download, EpisodeInfo, Playlist, Share, Transcript];

  function PodigeePodcastPlayer(elemClass) {
    this.elemClass = elemClass;
    this.sendSizeChange = bind(this.sendSizeChange, this);
    this.bindWindowResizing = bind(this.bindWindowResizing, this);
    this.initializeExtensions = bind(this.initializeExtensions, this);
    ...
    this.getConfiguration().loaded.done((function(_this) {
      return function() {
        _this.externalData = new ExternalData(_this);
  ...
  PodigeePodcastPlayer.prototype.getFeed = function() {
    ...
    return this.podcast.feed = new Feed(this);
  };

  PodigeePodcastPlayer.prototype.getProductionData = function() {
    var self;
    if (!this.episode.productionDataUrl) {
      return;
    }
    self = this;
    return $.getJSON(this.episode.productionDataUrl).done((function(_this) {
      return function(data) {
        return self.episode.productionData = data.data;
      };
    })(this));
  };
  ...
  PodigeePodcastPlayer.prototype.initPlayer = function() {
    var mediaElem;
    mediaElem = this.elem.find('audio')[0];
    return new Player(this, mediaElem, this.options, this.init);
  };
  ...
  PodigeePodcastPlayer.prototype.init = function(player) {
    ...
    window.setTimeout(this.sendSizeChange, 0);
    return this.bindWindowResizing();
  };

  PodigeePodcastPlayer.prototype.togglePlayState = function() {
    if (this.player == null) {
      return;
    }
    if (this.player.playing) {
      return this.elem.addClass('playing');
    } else {
      return this.elem.removeClass('playing');
    }
  };
  ...
  PodigeePodcastPlayer.prototype.adjustPlaySpeed = function(timeString) {
    ...
    if (!this.episode.productionData) {
      return;
    }
    currentTime = this.player.media.currentTime;
    item = $.grep(this.episode.productionData.statistics.music_speech, function(item, index) {
    ...
    if (item.length) {
      if (item[0].label === 'music') {
        if (this.options.currentPlaybackRate !== 1.0) {
          ...
          this.player.setPlaySpeed(1.0);
        }
      } else {
        if (this.tempPlayBackSpeed) {
          this.player.setPlaySpeed(this.tempPlayBackSpeed);
          this.tempPlayBackSpeed = null;
        }
      }
    }
    return this.updateSpeedDisplay();
  };
    ...
    this.theme.playPauseElement.click((function(_this) {
      return function() {
        if (_this.extensions.ChromeCast && _this.extensions.ChromeCast.active) {
          return _this.extensions.ChromeCast.togglePlayState();
        } else {
          return _this.player.playPause();
        }
      };
    })(this));
    this.theme.backwardElement.click((function(_this) {
      return function() {
        return _this.player.jumpBackward();
      };
    })(this));
    this.theme.forwardElement.click((function(_this) {
      return function() {
        return _this.player.jumpForward();
      };
    })(this));
    return this.theme.speedElement.click((function(_this) {
      return function(event) {
        _this.player.changePlaySpeed();
        return _this.updateSpeedDisplay();
      };
    })(this));
  };

  PodigeePodcastPlayer.prototype.updateSpeedDisplay = function() {
  ...
  PodigeePodcastPlayer.prototype.initializeExtensions = function() {
    ...
    return PodigeePodcastPlayer.defaultExtensions.forEach((function(_this) {
  ...
  PodigeePodcastPlayer.prototype.bindWindowResizing = function() {
    return $(window).on('resize', _.debounce(this.sendSizeChange, 250));
  };

  PodigeePodcastPlayer.prototype.sendSizeChange = function() {
    ...
    return this.extensions.ProgressBar.updateBarWidths();
  };

  PodigeePodcastPlayer.prototype.isInIframeMode = function() {
    return this.options.iframeMode === 'iframe';
  };

  return PodigeePodcastPlayer;
...
},{"./configuration.coffee":12,"./embed.coffee":13,"./extensions/chaptermarks.coffee":15,"./extensions/download.coffee":16,"./extensions/episode_info.coffee":17,"./extensions/playlist.coffee":18,"./extensions/share.coffee":19,"./extensions/transcript.coffee":20,"./external_data.coffee":23,"./feed.coffee":24,"./player.coffee":26,"./progress_bar.coffee":27,"./theme.coffee":28,"jquery":2,"lodash":3}],11:[function(require,module,exports){
var AudioFile, _;

_ = require('lodash');

AudioFile = (function() {
  AudioFile.formatMapping = {
    m4a: 'audio/mp4',
    mp3: 'audio/mpeg',
  ...
  AudioFile.reverseFormatMapping = _.invert(AudioFile.formatMapping);

  function AudioFile(format, uri, media) {
    this.uri = uri;
    this.media = media;
    this.format = format;
    this.findOutIfPlayable();
  }

  AudioFile.prototype.findOutIfPlayable = function() {
    return this.playable = this.uri ? this.media.canPlayType(this.fileFormat()) : '';
  };

  AudioFile.prototype.fileFormat = function() {
    return AudioFile.formatMapping[this.format];
  };
...
Configuration = (function() {
  function Configuration(app) {
    ...
    if (this.frameOptions.configuration) {
      this.configuration = {
        json_config: this.frameOptions.configuration
      };
  ...
  Configuration.prototype.receiveConfiguration = function() {
    $(window).on('message', (function(_this) {
      return function(event) {
        ...
        if (_this.configuration.json_config) {
    ...
    return window.parent.postMessage('sendConfig', '*');
  };

  Configuration.prototype.fetchJsonConfiguration = function() {
    var self;
    if (!(this.configuration.json_config && this.configuration.json_config.length)) {
      return;
    }
    self = this;
    return $.ajax({
      ...
      url: this.configuration.json_config
    }).done((function(_this) {
      return function(data) {
        self.configuration = _.extend(self.configuration, data);
        return self.setConfigurations(true);
      };
    })(this)).error(function(xhr, status, trace) {
      console.debug("[podigee podcast player] Error while fetching player configuration:");
      console.debug("xhr:", xhr);
      console.debug("status:", status);
      return console.debug("trace:", trace);
    });
  };

  Configuration.prototype.setConfigurations = function(viaJSON) {
    var base, base1;
    this.app.podcast = this.configuration.podcast || {};
    this.app.episode = this.configuration.episode;
    if (this.app.episode.cover_url != null) {
      console.warn('Please use episode.coverUrl instead of episode.cover_url in player configuration');
      if ((base = this.app.episode).coverUrl == null) {
        base.coverUrl = this.configuration.episode.cover_url;
      }
    }
    if ((base1 = this.app.episode).embedCode == null) {
      base1.embedCode = this.configuration.embedCode;
    }
    ...
    this.app.options.parentLocationHash = this.configuration.parentLocationHash;
    this.app.options.configViaJSON = viaJSON;
...
},{"./utils.coffee":29,"jquery":2,"lodash":3,"rivets":4}],13:[function(require,module,exports){
...
Iframe = (function() {
  function Iframe(elem1) {
    ...
    this.configuration = typeof config === 'string' ? window[config] || {} : config;
    if (_.isEmpty(this.configuration)) {
      this.configuration.json_config = config;
    }
    ...
    this.url = scriptPath + "/podigee-podcast-player.html?id=" + this.id + "&iframeMode=script";
    this.buildIframe();
    this.setupListeners();
    this.replaceElem();
...
},{"./iframe_resizer.coffee":25,"jquery":2,"lodash":3}],14:[function(require,module,exports){
var $, Extension,
  bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; };
...
Extension = (function() {
  ...
  Extension.prototype.defaultOptions = {};

  Extension.prototype.renderButton = function() {
    this.button = $(this.buttonHtml);
    return this.button.on('click', (function(_this) {
      return function() {
        return _this.app.theme.togglePanel(_this.panel);
      };
    })(this));
  };

  Extension.prototype.renderPanel = function() {
    this.panel = $(this.panelHtml);
    return this.panel.hide();
  };

  Extension.prototype.buttonHtml = "<button class=\"fa fa-bookmark\" title=\"Show example extension\"></button>";

  Extension.prototype.panelHtml = "<div class=\"example\">\n  <h3>Example</h3>\n\n  <p>This is an example</p>\n</div>";

  Extension.prototype.name = function() {
    return this.constructor.extension.name;
  };
...
var $, ChapterMark, ChapterMarks, Extension, Utils, _, rivets, sightglass,
  bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
...
ChapterMarks = (function(superClass) {
  extend(ChapterMarks, superClass);
  ...
  function ChapterMarks(app) {
    ...
    this.chaptermarks = this.app.episode.chaptermarks;
    ...
    this.renderButton();
    this.attachEvents();
    this.app.theme.addExtension(this);
  }
  ...
  ChapterMarks.prototype.click = function(event) {
    ...
    return this.app.player.media.currentTime = Utils.hhmmssToSeconds(time);
  };
  ...
  ChapterMarks.prototype.setActiveMark = function() {
    var time;
    time = this.app.player.currentTimeInSeconds;
  ...
  ChapterMarks.prototype.buttonHtml = "<button class=\"chaptermarks-button\" title=\"Show chaptermarks\"></button>";
...
},{"../extension.coffee":14,"../utils.coffee":29,"jquery":2,"lodash":3,"rivets":4,"sightglass":5}],16:[function(require,module,exports){
var $, Download, Extension, _, rivets, sightglass,
  bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
  extend = function(child, parent) { for (var key in parent) { if (hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
  hasProp = {}.hasOwnProperty;
...
Download = (function(superClass) {
  ...
  Download.prototype.buttonHtml = "<button class=\"download-button\" title=\"Download episode\"></button>";
...
},{"../extension.coffee":14,"jquery":2,"lodash":3,"rivets":4,"sightglass":5}],17:[function(require,module,exports){
var $, EpisodeInfo, Extension, _, rivets, sightglass,
  bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
  extend = function(child, parent) { for (var key in parent) { if (hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
  hasProp = {}.hasOwnProperty;
...
EpisodeInfo = (function(superClass) {
  extend(EpisodeInfo, superClass);
  ...
  EpisodeInfo.prototype.buttonHtml = "<button class=\"episode-info-button\" title=\"Show more info\"></button>";
...
},{"../extension.coffee":14,"jquery":2,"lodash":3,"rivets":4,"sightglass":5}],18:[function(require,module,exports){
var $, Extension, Playlist, PlaylistItem, _, rivets, sightglass,
  bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
  extend = function(child, parent) { for (var key in parent) { if (hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
  hasProp = {}.hasOwnProperty;
...
PlaylistItem = (function() {
  function PlaylistItem(context, callback) {
    this.render = bind(this.render, this);
...
Playlist = (function(superClass) {
  ...
  function Playlist(app) {
    ...
    this.feed.promise.done((function(_this) {
      return function() {
        _this.renderPanel();
        _this.renderButton();
        return _this.app.theme.addExtension(_this);
      };
    })(this));
  }
  ...
  Playlist.prototype.click = function(event) {
    this.updateEpisodeData(event.data);
    this.app.initializeExtensions();
    return this.app.player.loadFile();
  };

  Playlist.prototype.updateEpisodeData = function(feedItem) {
    this.app.episode.title = feedItem.title;
    this.app.episode.subtitle = feedItem.subtitle;
    this.app.episode.description = feedItem.description;
    this.app.episode.media = {
      mp3: feedItem.enclosure.mp3,
      m4a: feedItem.enclosure.m4a,
      ogg: feedItem.enclosure.ogg,
      opus: feedItem.enclosure.opus
    };
    this.app.episode.url = feedItem.href;
    this.app.episode.transcript = null;
    return this.app.episode.chaptermarks = null;
  };
  ...
  Playlist.prototype.renderPanel = function() {
    ...
    _.each(this.feed.items, (function(_this) {
      return function(feedItem, index) {
        var playlistItem;
  ...
  Playlist.prototype.buttonHtml = "<button class=\"playlist-button\" title=\"Show playlist\"></button>";
...
},{"../extension.coffee":14,"jquery":2,"lodash":3,"rivets":4,"sightglass":5}],19:[function(require,module,exports){
var $, Extension, Share, Uri, _, rivets, sightglass,
  bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
  extend = function(child, parent) { for (var key in parent) { if (hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
  hasProp = {}.hasOwnProperty;
...
Share = (function(superClass) {
  ...
  function Share(app) {
    ...
    this.attachEvents = bind(this.attachEvents, this);
    this.renderPanel = bind(this.renderPanel, this);
    this.shareUrl = bind(this.shareUrl, this);
    this.updateContext = bind(this.updateContext, this);
    this.buildContext = bind(this.buildContext, this);
    this.shareLinks = bind(this.shareLinks, this);
    this.episode = this.app.episode;
    if (!this.episode) {
      return;
    }
  ...
  Share.prototype.shareLinks = function(currentTimeInSeconds) {
    var coverUrl, fileUrl, shareLinks, title, url;
    url = encodeURI(this.shareUrl());
    fileUrl = this.audioFileUrl();
    title = encodeURI(this.episode.title);
    coverUrl = encodeURI(this.episode.coverUrl);
    return shareLinks = {
      clammr: this.clammrUrl(title, coverUrl, url, fileUrl, currentTimeInSeconds),
  ...
  Share.prototype.clammrUrl = function(title, coverUrl, url, fileUrl, time) {
    var popupUrl, startTime;
    if (!fileUrl) {
      return;
    }
    startTime = time > 24 ? (time - 24) * 1000 : 0;
    return popupUrl = "https://www.clammr.com/app/clammr/crop\n?audioUrl=" + fileUrl + "\n&referralName=\"Podigee Podcast Player\"\n&title=" + title + "\n&imageUrl=" + coverUrl + "\n&attributeUrl=" + url + "\n&extendedUrl=" + url + "\n&audioStartTime=" + startTime;
  };

  Share.prototype.audioFileUrl = function() {
    var url;
    url = this.app.episode.media.mp3 || this.app.episode.media.m4a;
    return encodeURI(url);
  };

  Share.prototype.buildContext = function() {
    var base;
    if (this.context == null) {
      this.context = {};
    }
    this.context.currentTime = this.app.player.currentTime;
    this.context.currentTimeInSeconds = this.app.player.currentTimeInSeconds;
    this.context.shareLinks = this.shareLinks(this.context.currentTimeInSeconds);
    this.context.url = this.shareUrl();
    if ((base = this.context).showUrlWithTime == null) {
      base.showUrlWithTime = false;
    }
    this.context.updateContext = this.updateContext;
    this.context.embedCode = this.app.episode.embedCode;
    return this.context.showEmbedUrl = this.app.options.configViaJSON;
  };

  Share.prototype.updateContext = function() {
    return this.buildContext();
  };

  Share.prototype.shareUrl = function() {
    var parsed, ref, time;
    parsed = Uri(this.episode.url);
    if ((ref = this.context) != null ? ref.showUrlWithTime : void 0) {
      time = Math.round(this.context.currentTimeInSeconds);
      return parsed.fragment("t=" + time);
    } else {
  ...
  Share.prototype.renderPanel = function() {
    this.panel = $(this.panelHtml);
    rivets.bind(this.panel, this.context);
    this.panel.hide();
    return this.bindEvents();
  };

  Share.prototype.attachEvents = function() {
    return $(this.app.player.media).on('timeupdate', this.buildContext);
  };

  Share.prototype.bindEvents = function() {
    this.panel.find('.share-copy-url').on('focus', this.copyUrlAction);
    this.panel.find('.share-embed-code').on('focus', this.copyUrlAction);
    return this.panel.find("[name='enable-start-at']").on('change', this.toggleStartAt);
  };
  ...
  Share.prototype.buttonHtml = "<button class=\"share-button\" title=\"Share episode URL\"></button>";
...
},{"../extension.coffee":14,"jquery":2,"lodash":3,"rivets":4,"sightglass":5,"urijs":8}],20:[function(require,module,exports){
var $, Extension, Search, Transcript, TranscriptLine, Utils, _, rivets, sightglass,
  bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
  extend = function(child, parent) { for (var key in parent) { if (hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
  hasProp = {}.hasOwnProperty;
...
Transcript = (function(superClass) {
  ...
  function Transcript(app) {
    ...
    this.load().done((function(_this) {
      return function() {
        _this.renderPanel();
        _this.renderButton();
        _this.app.theme.addExtension(_this);
  ...
  Transcript.prototype.transcriptFileFormat = function() {
    return _.last(this.transcript.split('.'));
  };
  ...
  Transcript.prototype.load = function() {
    var promise;
    promise = this.app.externalData.get(this.transcript);
  ...
  Transcript.prototype.parseSrt = function(raw) {
    var segments, splitBy;
    splitBy = raw.search("\n\r\n") > -1 ? "\n\r\n" : "\n\n";
  ...
  Transcript.prototype.bindEvents = function() {
    ...
    this.panel.find('li').click((function(_this) {
      return function(event) {
        return _this.app.player.media.currentTime = event.currentTarget.dataset.timestamp;
      };
    })(this));
    return this.search.initInterface(this, this.panel);
  };
  ...
  Transcript.prototype.scrollToLine = function(elem) {
    if (!elem) {
      return;
    }
    return this.panel.find('ul').scrollTop(elem.offsetTop - 50);
  };
  ...
  Transcript.prototype.buttonHtml = "<button class=\"transcript-button\" title=\"Show transcript\"></button>";
...
},{"../extension.coffee":14,"../utils.coffee":29,"./transcript/line.coffee":21,"./transcript/search.coffee":22,"jquery":2,"lodash":3,"rivets":4,"sightglass":5}],21:[function(require,module,exports){
var $, TranscriptLine, rivets, sightglass,
  bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; };
...
},{"jquery":2,"rivets":4,"sightglass":5}],22:[function(require,module,exports){
var $, TranscriptSearch, _, elasticlunr, rivets, sightglass;
...
TranscriptSearch = (function() {
  ...
  TranscriptSearch.prototype.setupIndex = function() {
    this.index = elasticlunr(function() {
      this.addField('time');
      this.addField('speaker');
      this.addField('text');
      return this.setRef('timestamp');
    });
    return this.index.pipeline.reset();
  };

  TranscriptSearch.prototype.addLine = function(line) {
    return this.index.addDoc(line.data);
  };

  TranscriptSearch.prototype.find = function(query) {
  ...
  TranscriptSearch.prototype.results = [];

  TranscriptSearch.prototype.initInterface = function(transcripts, elem) {
    var elems, html, nextResult, prevResult, searchClear, searchInput, searchInterface, searchNextResult, searchPrevResult, searchResult, searchResultSize, transcriptLines;
    ...
    elems = [];
    nextResult = (function(_this) {
      return function() {
        if (_this.currentSearchResultIndex < _this.results.length) {
          _this.currentSearchResultIndex = _this.currentSearchResultIndex + 1;
        } else {
          _this.currentSearchResultIndex = 1;
        }
        transcripts.scrollToLine(elems[_this.currentSearchResultIndex - 1]);
        return _this.updateSearchResult();
      };
    })(this);
    prevResult = (function(_this) {
      return function() {
        if (_this.currentSearchResultIndex > 1) {
          _this.currentSearchResultIndex = _this.currentSearchResultIndex - 1;
        } else {
          _this.currentSearchResultIndex = _this.results.length;
        }
        transcripts.scrollToLine(elems[_this.currentSearchResultIndex - 1]);
        return _this.updateSearchResult();
      };
    })(this);
    searchInput.on('keyup', (function(_this) {
      return function(event) {
        if (event.keyCode === 13) {
          return nextResult();
        }
        _this.query = _this.data.query = event.target.value;
        _this.results = _this.find(_this.query);
        transcriptLines.removeClass('search-highlight');
        if (_this.results.length) {
          elems = _this.results.map((function(result, index) {
            elem = _.find(transcriptLines, (function(_this) {
              return function(line) {
                return line.dataset.timestamp === result.ref;
              };
            })(this));
            return elem;
          }), _this);
          ...
          return searchNextResult.off('click').on('click', nextResult);
        } else {
          _this.currentSearchResultIndex = 1;
          return _this.updateSearchResult();
        }
    ...
    return searchClear.on('click', function(event) {
      searchInput.val(null);
      return searchInput.trigger('keyup');
    });
  };
...
},{"elasticlunr":1,"jquery":2,"lodash":3,"rivets":4,"sightglass":5}],23:[function(require,module,exports){
var $, ExternalData;

$ = require('jquery');

ExternalData = (function() {
  function ExternalData(app) {
    this.sslProxy = app.options.sslProxy;
  }

  ExternalData.prototype.get = function(url) {
    var deferred;
    deferred = $.Deferred();
    url = this.sslProxy ? "" + this.sslProxy + url : url;
...
FeedItem = (function() {
  function FeedItem(xml) {
    this.xml = xml;
    this.mapEnclosure = bind(this.mapEnclosure, this);
  ...
  FeedItem.prototype.parse = function() {
    ...
    this.enclosure = this.mapEnclosure();
    return this.description = this.extract('description').html();
  };

  FeedItem.prototype.extract = function(elemName) {
    return this[elemName] != null ? this[elemName] : this[elemName] = $(this.xml).find(elemName);
  };

  FeedItem.prototype.mapEnclosure = function() {
    var enclosure, media, type, url;
    enclosure = this.extract('enclosure');
    url = enclosure.attr('url');
    type = enclosure.attr('type');
    media = {};
    media[this.enclosureMapping(type)] = url;
    return media;
  };

  FeedItem.prototype.enclosureMapping = function(type) {
    return AudioFile.reverseFormatMapping[type];
  };
...
Feed = (function() {
  function Feed(app) {
    var deferred;
    if (app.podcast.feed.constructor !== Feed) {
      this.feedUrl = app.podcast.feed;
      this.externalData = app.externalData;
      this.fetch();
    } else {
      this.feed = app.podcast.feed.feed;
      this.items = app.podcast.feed.items;
      deferred = $.Deferred();
  ...
  Feed.prototype.fetch = function() {
    var self;
    self = this;
    this.promise = this.externalData.get(this.feedUrl);
...
},{"./audio_file.coffee":11,"jquery":2}],25:[function(require,module,exports){
var $, IframeResizer;
...
IframeResizer = (function() {
  ...
  IframeResizer.listen = function(listenTo, iframe, offset, callback) {
    ...
    return $(window).on('message', (function(_this) {
      return function(event) {
        var error, height, resizeData, width;
        try {
...
Player = (function() {
  function Player(app, elem) {
    var self;
    this.app = app;
    this.timeHash = bind(this.timeHash, this);
    this.playPause = bind(this.playPause, this);
    this.setPlaySpeed = bind(this.setPlaySpeed, this);
    this.setDuration = bind(this.setDuration, this);
    this.setCurrentTime = bind(this.setCurrentTime, this);
    this.setInitialTime = bind(this.setInitialTime, this);
    this.attachEvents = bind(this.attachEvents, this);
    this.loadFile = bind(this.loadFile, this);
    this.changePlaySpeed = bind(this.changePlaySpeed, this);
    this.jumpForward = bind(this.jumpForward, this);
  ...
  Player.prototype.jumpBackward = function(seconds) {
    seconds = seconds || this.app.options.backwardSeconds;
    return this.media.currentTime = this.media.currentTime - seconds;
  };

  Player.prototype.jumpForward = function(seconds) {
    seconds = seconds || this.app.options.forwardSeconds;
    return this.media.currentTime = this.media.currentTime + seconds;
  };

  Player.prototype.changePlaySpeed = function() {
    var nextRateIndex;
    nextRateIndex = this.app.options.playbackRates.indexOf(this.app.options.currentPlaybackRate) + 1;
    if (nextRateIndex >= this.app.options.playbackRates.length) {
      nextRateIndex = 0;
    }
    return this.setPlaySpeed(this.app.options.playbackRates[nextRateIndex]);
  };

  Player.prototype.loadFile = function() {
    var files;
    this.pause();
    files = _.map(this.app.episode.media, (function(_this) {
      return function(uri, format) {
        return new AudioFile(format, uri, _this.media);
      };
    })(this));
    files = this.filterUnplayable(files);
    files = this.sortByPlayability(files);
    files = this.sortByFormat(files);
    this.media.src = files[0].uri;
    return this.setDuration();
  };

  Player.prototype.filterUnplayable = function(files) {
    return _.filter(files, function(file) {
      return file.playable !== '';
    });
  };

  Player.prototype.sortByPlayability = function(files) {
    return _.sortBy(files, function(file) {
      if (file.playable === '') {
        return 1;
      }
      if (file.playable === 'maybe' && file.playable === 'probably') {
        return 1;
      }
      if (file.playable === 'probably') {
        return -1;
      }
      return 0;
    });
  ...
  Player.prototype.sortByFormat = function(files) {
    return _.sortBy(files, function(file) {
      if (file.playable === 'probably' && file.format === 'opus') {
        return -100;
      }
      if (file.playable === 'probably' && file.format === 'm4a') {
        return -90;
      }
      return 0;
    });
  ...
  Player.prototype.attachEvents = function() {
    return $(this.media).on('timeupdate', this.setCurrentTime);
  };

  Player.prototype.setInitialTime = function() {
    return this.media.currentTime = this.timeHash();
  };

  Player.prototype.setCurrentTime = function() {
    this.currentTimeInSeconds = this.media.currentTime;
    return this.currentTime = Utils.secondsToHHMMSS(this.currentTimeInSeconds);
  };
  ...
  Player.prototype.setDuration = function() {
    ...
    return interval = window.setInterval(((function(_this) {
      return function(t) {
        if (!(_this.media.readyState > 0)) {
          return;
        }
        _this.app.episode.duration = Utils.secondsToHHMMSS(_this.media.duration);
        return clear();
      };
    })(this)), 500);
  };
  ...
  Player.prototype.setPlaySpeed = function(speed) {
    return this.media.playbackRate = this.app.options.currentPlaybackRate = speed;
  };
  ...
  Player.prototype.play = function() {
    ...
    this.media.play();
    this.playing = true;
    return this.app.togglePlayState();
  };
  ...
  Player.prototype.timeHash = function() {
    var hash, timeHash;
    if (hash = this.app.options.parentLocationHash) {
      hash = hash.slice(1).split('&');
      timeHash = _(hash).find(function(h) {
        return _(h).startsWith('t');
      });
      if (timeHash) {
        return timeHash.split('=')[1];
      } else {
        return 0;
      }
    } else {
...
},{"./audio_file.coffee":11,"./utils.coffee":29,"jquery":2,"lodash":3}],27:[function(require,module,exports){
...
ProgressBar = (function() {
  ...
  function ProgressBar(app) {
    ...
    this.updateTime = bind(this.updateTime, this);
    this.updateBarWidths = bind(this.updateBarWidths, this);
  ...
  ProgressBar.prototype.showBuffering = function() {
    return this.bufferingElement.show();
  };

  ProgressBar.prototype.hideBuffering = function() {
    return this.bufferingElement.hide();
  };

  ProgressBar.prototype.switchTimeDisplay = function() {
    this.timeMode = this.timeMode === 'countup' ? 'countdown' : 'countup';
    return this.updateTime();
  };

  ProgressBar.prototype.updateBarWidths = function() {
    this.updatePlayed();
    return this.updateLoaded();
  };

  ProgressBar.prototype.updateTime = function() {
  ...
  ProgressBar.prototype.findElements = function() {
    ...
    this.railElement = this.elem.find('.progress-bar-rail');
    this.playedElement = this.elem.find('.progress-bar-played');
    this.loadedElement = this.elem.find('.progress-bar-loaded');
  ...
  ProgressBar.prototype.jumpToPosition = function(position) {
  ...
  ProgressBar.prototype.barWidth = function() {
    return this.railElement.width();
  };

  ProgressBar.prototype.timeRailFactor = function() {
    return this.barWidth() / this.player.duration;
  };

  ProgressBar.prototype.updatePlayed = function() {
    var newWidth;
    newWidth = this.player.currentTime * this.timeRailFactor();
    return this.playedElement.width(newWidth);
  };
...
},{"./utils.coffee":29,"jquery":2,"rivets":4,"sightglass":5}],28:[function(require,module,exports){
...
Theme = (function() {
  function Theme(app) {
    ...
    this.addExtension = bind(this.addExtension, this);
    ...
    this.loadCss = bind(this.loadCss, this);
    this.loadHtml = bind(this.loadHtml, this);
    this.render = bind(this.render, this);
  ...
  Theme.prototype.render = function() {
    ...
    this.addEmbedModeClass();
    this.findElements();
    this.bindCoverLoad();
    return this.elem;
  };

  Theme.prototype.loadHtml = function() {
    var loaded, self;
    loaded = $.Deferred();
    self = this;
    if (this.html == null) {
  ...
  Theme.prototype.addEmbedModeClass = function() {
    var modeClass;
    modeClass = "mode-" + this.app.options.iframeMode;
    return this.elem.addClass(modeClass);
  };
  ...
  Theme.prototype.findElements = function() {
    ...
    this.panels = this.elem.find('.panels');
    if (!this.app.isInIframeMode()) {
      return this.panels.hide();
    }
  };
  ...
  Theme.prototype.bindCoverLoad = function() {
    return this.coverImage.on('load', (function(_this) {
      return function() {
        return _this.app.sendSizeChange();
      };
    })(this));
  };
  ...
  Theme.prototype.addExtension = function(extension) {
    this.addButton(extension.button);
    this.panels.append(extension.panel);
    if (extension.name() === this.app.options.startPanel) {
      extension.button.trigger('click');
    }
    if (!this.app.options.startPanel && this.app.isInIframeMode()) {
      this.buttons.hide();
      return this.panels.hide();
    }
  };

  Theme.prototype.animationOptions = function() {
    return {
      duration: 300,
      step: _.debounce(this.app.sendSizeChange, 50)
    };
  };

  Theme.prototype.activePanel = null;

  Theme.prototype.togglePanel = function(elem) {
    if (this.activePanel != null) {
      if (this.activePanel === elem) {
        if (!this.app.isInIframeMode()) {
          this.activePanel.slideToggle(this.animationOptions());
          this.panels.slideToggle(this.animationOptions());
          return this.activePanel = null;
        }
      } else {
        this.activePanel.slideToggle(this.animationOptions());
        elem.slideToggle(this.animationOptions());
        return this.activePanel = elem;
      }
    } else {
      if (!this.app.isInIframeMode()) {
        this.panels.slideToggle(this.animationOptions());
...
},{"jquery":2,"lodash":3,"rivets":4,"sightglass":5}],29:[function(require,module,exports){
var Utils;
...
Utils = (function() {
  ...
  Utils.hhmmssToSeconds = function(string) {
  ...
  Utils.padNumber = function(number, length) {
    if (length == null) {
      length = 2;
    }
